...
import com.smartstadium.dto.ChatRequest;
...
@Tag(
    name = "AI Assistant",
    description = "Vertex AI powered natural language queries"
)
public class AiAssistantController {
    ...
    @Operation(
        ...
        description = "Accepts natural language constraints and returns an AI-generated suggestion."
    )
    public ResponseEntity<Map<String, String>> chat(
        @Valid @RequestBody ChatRequest request
    ) {
        String message =
            request.message() != null ? request.message().toLowerCase() : "";
        ...
        return ResponseEntity.ok(
            Map.of("response", responseMessage, "role", "assistant")
        );
    }
    ...
    private String callVertexAI(String userMessage) {
        try (VertexAI vertexAI = new VertexAI(projectId, location)) {
            ...
            String stadiumContext = crowdService
                .getAllDensities()
                .stream()
                .map(d ->
                    String.format(
    ...
    private String generateMockResponse(String message) {
        if (
            ...
            if (north.getCurrentCount() < south.getCurrentCount()) {
                return (
                    "The North Restroom currently has fewer people (" +
                    north.getCurrentCount() +
                    ") compared to the South Restroom. I recommend heading there!"
                );
            } else {
                return (
                    "The South Restroom currently has fewer people (" +
                    south.getCurrentCount() +
                    ") compared to the North Restroom. I recommend heading there!"
                );
            }
        }
