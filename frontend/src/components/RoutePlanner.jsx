...
/**
 * List of all stadium zones for dropdowns.
 */
const ZONES = [
  ...
  return (
    <section className="card" aria-label="Route Planner">
      <div className="card-header">
        <h2 className="card-title"><span className="icon" aria-hidden="true">🧭</span> Route Planner</h2>
      </div>

      <div className="route-form" role="search" aria-label="Find optimal route">
      ...
      {route && (
        <div className="route-result" aria-live="polite" aria-label="Route result">
          <div className="route-info">
            <div className="route-stat">
              <div className="route-stat-value">{formatTime(route.estimatedTimeSeconds)}</div>
              <div className="route-stat-label">Est. Travel Time</div>
            </div>
            <div className="route-stat">
              <div className="route-stat-value">{route.path.length - 1}</div>
              <div className="route-stat-label">Steps</div>
            </div>
            <div className="route-stat">
              <div className="route-stat-value">{route.totalWeight}m</div>
      ...
      {!route && !error && !loading && (
        <p className="empty-state">Select origin and destination to find the best route</p>
      )}
    </section>
  );
}
